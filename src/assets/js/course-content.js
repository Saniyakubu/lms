...
async function getCourseContent() {
  try {
    const response = await fetch(
      `${url}student/get-course-content.php?courseId=${id}`
    );
    ...
    if (progressArray.length > 0) {
      const lastVideoProgress = progressArray.find(
      ...
      localStorage.setItem(
        'currentVideo',
        JSON.stringify({ course: id, videoId: courseContents[0].id })
      );
    }
    ...
    if (questionAnswers.length > 0) {
      ...
      container.innerHTML =
        '<p>No questions and answers found for the course.</p>';
    }
